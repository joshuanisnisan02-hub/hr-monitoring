...
class HrRebuiltApp extends StatelessWidget {
  const HrRebuiltApp({super.key});
...
class AppSidebar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const items = [
      ...
      NavItem('References', Icons.tune_rounded),
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: const Text(
              'Use References to add missing choices for contracts, credentials, evaluations, and ranking.',
...
Future<List<dynamic>> loadReferences({int limit = 1500}) => db.from('hr_reference_options').select('id, category, label, value, salary, is_active, sort_order').order('category').order('sort_order').order('label').limit(limit);
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(spacing: 14, runSpacing: 14, children: [
                  ...
                  QuickCard('Manage References', Icons.tune_rounded, () => onNavigate(6)),
                  QuickCard('Print Reports', Icons.print_rounded, () => onNavigate(7)),
                ]),
              ],
            ),
          );
...
class ReferencesPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => PageFrame(
        ...
        child: CrudTable(
          ...
          columns: const [
            GridCol('category', 'Category', flex: 2, primary: true),
            GridCol('label', 'Display Label', flex: 3),
            GridCol('value', 'Saved Value', flex: 3),
            GridCol('salary', 'Salary', isMoney: true),
            GridCol('is_active', 'Active', isBool: true),
            GridCol('sort_order', 'Sort', isNumber: true),
          ],
...
class GridCol {
  ...
  const GridCol(this.key, this.label, {this.flex = 1, this.primary = false, this.isStatus = false, this.isMoney = false, this.isNumber = false, this.isBool = false});
}
  ...
  Future<void> confirmDelete(BuildContext context, Map<String, dynamic> row) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('This will remove ${formatValue(valueFor(row, widget.columns.first.key))}.'),
        actions: [
...
Widget tableCell(GridCol col, Object? raw) {
  ...
  if (col.isBool) return Align(alignment: Alignment.centerLeft, child: StatusChip(raw == true ? 'Active' : 'Inactive'));
...
class ReadOnlyBox extends StatelessWidget {
  ...
  const ReadOnlyBox({super.key, required this.label, required this.value});
  ...
  Widget build(BuildContext context) => SizedBox(
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(label, style: const TextStyle(color: Color(0xFF1E40AF), fontWeight: FontWeight.w700, fontSize: 12)),
...
Future<List<EditOption>> referenceOptions(String category, List<EditOption> fallback) async {
  try {
    ...
    final refs = rows.map<EditOption>((r) => EditOption((r['value'] ?? r['label']).toString(), formatValue(r['label']), salary: num.tryParse('${r['salary'] ?? ''}'))).toList();
...
String linkedEmployeeName(Map<String, dynamic>? row) => row?['employees'] is Map ? formatValue(row?['employees']['full_name']) : 'Linked Employee';
...
Future<void> editReference(BuildContext context, Map<String, dynamic>? row, VoidCallback refresh) async {
  ...
  if (data == null) return;
  data['value'] = emptyToNull('${data['value'] ?? ''}') ?? data['label'];
    ...
    final selected = await showDialog<EditOption>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: SizedBox(
          ...
          height: 440,
          child: ranks.isEmpty
              ? const Center(child: Text('No rank reference found. Add ranks under References.'))
              : ListView.separated(
          ...
          child: Form(
            ...
            child: SingleChildScrollView(
              child: Wrap(spacing: 14, runSpacing: 14, children: [
                ...
                ReadOnlyBox(label: 'Previous Rank', value: formatValue(initial?['previous_rank_text'])),
                ReadOnlyBox(label: 'Previous Salary', value: formatMoney(initial?['previous_salary'])),
                ...
                SizedBox(
                  ...
                  child: Container(
                    ...
                    child: const Text('Previous rank and previous salary are locked. Add new rank choices under References.', style: TextStyle(color: Color(0xFF1E3A8A), fontWeight: FontWeight.w600)),
                  ),
                ),
              ]),
            ),
...
class _ReportsPageState extends State<ReportsPage> {
  ...
  List<ReportConfig> get reports => [
        ReportConfig('Employee Master List', () => loadEmployees(limit: 5000), const [GridCol('full_name', 'Employee Name', flex: 3, primary: true), GridCol('appointment', 'Appointment', flex: 2), GridCol('designation', 'Designation', flex: 2), GridCol('employee_type', 'Type'), GridCol('employment_status', 'Status'), GridCol('current_salary', 'Salary', isMoney: true)]),
        ReportConfig('Contract Monitoring Report', () => loadContracts(limit: 5000), const [GridCol('employee_name', 'Employee Name', flex: 3, primary: true), GridCol('contract_type', 'Contract Type', flex: 2), GridCol('status', 'Status'), GridCol('contract_start_date', 'Start'), GridCol('contract_end_date', 'End'), GridCol('days_left', 'Days Left', isNumber: true)]),
        ReportConfig('License Report', () => loadLicenses(limit: 5000), const [GridCol('employee_name', 'Employee Name', flex: 3, primary: true), GridCol('license_name', 'License', flex: 2), GridCol('license_number', 'License No.', flex: 2), GridCol('expiry_date', 'Expiry'), GridCol('status', 'Status')]),
        ReportConfig('National Certificate Report', () => loadCertificates(limit: 5000), const [GridCol('employee_name', 'Employee Name', flex: 3, primary: true), GridCol('certificate_name', 'Certificate', flex: 3), GridCol('certificate_type', 'Type', flex: 2), GridCol('expiry_date', 'Expiry'), GridCol('status', 'Status')]),
        ReportConfig('Evaluation Report', () => loadEvaluations(limit: 5000), const [GridCol('employee_name', 'Employee Name', flex: 3, primary: true), GridCol('academic_year', 'A.Y.'), GridCol('semester', 'Semester'), GridCol('total_rating', 'Total', isNumber: true), GridCol('total_description', 'Description', flex: 2)]),
        ReportConfig('Ranking Report', () => loadRankings(limit: 5000), const [GridCol('employee_name', 'Employee Name', flex: 3, primary: true), GridCol('cycle_name', 'Cycle', flex: 2), GridCol('previous_rank_text', 'Previous', flex: 2), GridCol('applied_rank_text', 'Applied', flex: 2), GridCol('points_earned', 'Points', isNumber: true), GridCol('approved_rank_text', 'Approved', flex: 2), GridCol('approved_salary', 'Salary', isMoney: true)]),
      ];
  ...
  Widget build(BuildContext context) => Card(
        child: ClipRRect(
          ...
          child: Column(children: [
            ...
            const Divider(height: 1, color: _line),
            Expanded(child: rows.isEmpty ? const EmptyBox() : ListView.separated(itemCount: rows.length, separatorBuilder: (_, __) => const Divider(height: 1, color: _line), itemBuilder: (_, i) => ReportRow(row: rows[i], columns: columns, index: i))),
          ]),
        ),
      );
}
...
String buildPrintableReportHtml(String title, List<GridCol> columns, List<Map<String, dynamic>> rows) {
  ...
  final body = rows.map((row) => '<tr>${columns.map((c) => '<td>${escapeHtml(displayForReport(c, valueFor(row, c.key)))}</td>').join()}</tr>').join();
  return '''<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>@page{size:A4 landscape;margin:10mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#0f172a;margin:0}h1{font-size:20px;margin:0 0 4px}.meta{color:#475569;font-size:11px;margin-bottom:14px}table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:10px}th{text-align:left;background:#f1f5f9;font-weight:700}th,td{border:1px solid #cbd5e1;padding:6px;vertical-align:top;word-wrap:break-word}tr:nth-child(even)td{background:#f8fafc}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style><script>window.addEventListener('load',function(){setTimeout(function(){window.focus();window.print();},350);});</script></head><body><h1>${escapeHtml(title)}</h1><div class="meta">Generated $date • ${rows.length} records</div><table><thead><tr>$header</tr></thead><tbody>$body</tbody></table></body></html>''';
}

String displayForReport(GridCol col, Object? raw) => col.isMoney ? formatMoney(raw) : col.isNumber ? formatNumber(raw) : formatValue(raw);
String escapeHtml(Object? value) => (value?.toString() ?? '').replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;').replaceAll('"', '&quot;').replaceAll("'", '&#39;');
