...
if ($markerIndex -lt 0) { throw 'Could not find SelectedLicenseInput marker in lib/main.dart.' }

$updatedStart = $text.LastIndexOf('const _contractTypeOptions', $markerIndex)
$legacyStart = $text.LastIndexOf('Future<void> editContract', $markerIndex)
if ($updatedStart -ge 0) {
  $start = $updatedStart
} elseif ($legacyStart -ge 0) {
  $start = $legacyStart
} else {
  throw 'Could not find editContract block in lib/main.dart.'
}
...
$text = $text.Substring(0, $start) + $replacement + $text.Substring($markerIndex)

if ($text -ne $original) {
  $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
  [System.IO.File]::WriteAllText($mainDart, $text, $utf8NoBom)
  Write-Host 'Applied contract modal update to lib/main.dart.'
} else {
