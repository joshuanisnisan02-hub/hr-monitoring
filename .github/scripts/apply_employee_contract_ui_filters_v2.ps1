...
function Find-CallEnd {
  param([string]$Source, [int]$ParenStart)
  if ($ParenStart -lt 0 -or $Source[$ParenStart] -ne '(') { return -1 }
  $depth = 0
  $quote = [char]0
  $escape = $false
  for ($i = $ParenStart; $i -lt $Source.Length; $i++) {
    $ch = $Source[$i]
    if ($quote -ne [char]0) {
      if ($escape) { $escape = $false }
      elseif ($ch -eq '\') { $escape = $true }
...
# Repair named parameter damage from older scripts if present.
...
$text = $text.Replace("Align:`r`n", "Align(`r`n")
...
# Safely remove the Employee Matching Card from EmployeesPage using parenthesis matching.
...
if ($empStart -ge 0 -and $empEnd -gt $empStart) {
  ...
  if ($marker -ge 0) {
    ...
    if ($cardStart -lt 0) { $cardStart = $emp.LastIndexOf('Card(', $marker) }
    if ($cardStart -ge 0) {
      $parenStart = $emp.IndexOf('(', $cardStart)
      $cardEnd = Find-CallEnd -Source $emp -ParenStart $parenStart
...
$text = $text.Replace('load: () => activeOnlyRows(activeOnlyRows(loadContracts())),', 'load: () => activeOnlyRows(loadContracts()),')
...
Write-Host 'Applied Employees and Contracts modifications safely:'
