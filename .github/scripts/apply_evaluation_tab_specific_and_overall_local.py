...
def remove_top_level_function(signature: str) -> None:
    global text
    while True:
...
# Clean leftover broken helper code that may have been left by earlier evaluation patches.
...
# Remove old evaluation helpers outside the EvaluationsPage block, if any.
for sig in [
...
Future<Map<String, dynamic>?> showEvaluationKindOnlyDialog(
  ...
  final rating = TextEditingController(text: formatEditValue(normalized[ratingKey]));
          ...
          child: Form(
            ...
            child: SingleChildScrollView(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                evaluationRatingBox(kind, rating, description, recompute, setDialogState),
              ]),
            ),
          ),
        ),
...
Future<void> viewEvaluationForKind(
  ...
  await showDialog<void>(
    ...
    builder: (_) => AlertDialog(
      title: Text('${evaluationKindTitle(kind)} - ${formatValue(normalized['employee_name'])}'),
      content: SizedBox(
        width: 680,
        child: Wrap(spacing: 10, runSpacing: 10, children: [
          ...
          DetailTile('Description',
              evaluationDescription(normalized, kind, ratingKey, descriptionKey)),
        ]),
      ),
      actions: [
...
Future<void> viewEvaluation(BuildContext context, Map<String, dynamic> row) async {
  ...
  await showDialog<void>(
    context: context,
    builder: (_) => AlertDialog(
      title: Text('Overall Evaluation - ${formatValue(normalized['employee_name'])}'),
      content: SizedBox(
        width: 850,
        child: SingleChildScrollView(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
...
Future<void> editEvaluationForKind(
    BuildContext context, Map<String, dynamic> row, VoidCallback refresh, EvaluationKind kind) async {
...
if text == original:
    print('No changes applied. Evaluation tabs may already be updated.')
else:
    print('Applied tab-specific Evaluation view/edit and added read-only Overall tab.')
